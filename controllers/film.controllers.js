...
const uploadFilm = async (req,res)=>{
    ...
    if(process.env.NODE_ENV === "production"){
        ...
        imgurl = `http://localhost:4000\\${img.slice(img.indexOf("public"))}`
        trailerurl = `http://localhost:4000\\${trailer.slice(trailer.indexOf("public"))}`
        srcurl = `http://localhost:4000\\${src.slice(src.indexOf("public"))}`
    }
